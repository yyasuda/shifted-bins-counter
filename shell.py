...
def insertSrcTable(srcMac):
    print("## INSERT SRC ## src={0}" 
            .format(mac2str(srcMac)))
    ...
    table_entry.table_id = context.get_obj_id(P4Type.table, "MyIngress.l2_src_table") 
    m1 = p4runtime_pb2.FieldMatch()
    m1.field_id = context.get_mf_id("MyIngress.l2_src_table", "hdr.ethernet.srcAddr")
    m1.exact.value = srcMac
    table_entry.match.append(m1)
    action = table_entry.action.action
    action.action_id = context.get_obj_id(P4Type.action, "MyIngress.nop") 

    # print(req)
    client.write(req)

def insertDstTable(dstMac, port):
    print("## INSERT DST ## dst={0} port={1}" 
            .format(mac2str(dstMac), int.from_bytes(port, byteorder='big')))
    ...
    table_entry = update.entity.table_entry
    table_entry.table_id = context.get_obj_id(P4Type.table, "MyIngress.l2_dst_table") 
    m1 = p4runtime_pb2.FieldMatch()
    m1.field_id = context.get_mf_id("MyIngress.l2_dst_table", "hdr.ethernet.dstAddr")
...
def packetin_process(pin):
    ...
    1. set reported mac + port information to l2_dst_table
    2. packetOut original payload with original ingress port
    """
...
def PacketOut(port, mcast_grp, payload):
    ...
    metadata.metadata_id = context.get_cpm_id("packet_out", "egress_port") 
    metadata.value = port
    packet.metadata.append(metadata)
    metadata.metadata_id = context.get_cpm_id("packet_out", "mcast_grp") 
...
def DumpTable(table):
    ...
    for te in table_entry.read():   
        match_key = te.match  
        odict = match_key._mk
        for k, v in odict.items():  
            key = k # MyIngress.l2_src_table # useless this time 
            value = v # MAC address 
        action = te.action 
        action_name = action.action_name # MyIngress.nop 
        params = action.msg().params 
        if len(params) == 0: 
            paramStr = "" 
        else: 
...
def ReadCounters(counter_id=None, index=None, dry_run=False):
    ...
    if dry_run:
        print("P4Runtime Read: {0}".format(request))
    else:
        for response in client.stub.Read(request):
            yield response
...
def Test():
    ...
    # print counter
    for response in ReadCounters(counter_id=302039124):
        for entity in response.entities:
            counter = entity.counter_entry
            print("#  {0}: {1} packets ({2} bytes)".format(counter.index.index, counter.data.packet_count, counter.data.byte_count))
    return

    # reset counter (need to specify index)
    req = p4runtime_pb2.WriteRequest()  
    update = req.updates.add()
    update.type = p4runtime_pb2.Update.MODIFY
    counter_entry = update.entity.counter_entry
    counter_entry.counter_id = 302012433
    counter_entry.index.index = 2
    print(req)
    client.write(req)
    return

    # counter reading message
    request = p4runtime_pb2.ReadRequest()
    request.device_id = client.device_id 
    entity = request.entities.add()
    counter_entry = entity.counter_entry
    counter_entry.counter_id = 302012433
    response = client.stub.Read(request)
    print(response)
    return

    # context feature adding
    ...
    print(context.get_mf_id("MyIngress.l2_src_table", "hdr.ethernet.dstAddr"))
...
"""
    ...
    m1 = p4runtime_pb2.FieldMatch()
    m1.field_id = context.get_mf_id("MyIngress.l2_src_table", "hdr.ethernet.srcAddr") # 1 
    m1.exact.value = b'\x00\x00\x00\x00\x00\x01' 
    m2 = p4runtime_pb2.FieldMatch()
    m2.field_id = context.get_mf_id("MyIngress.l2_dst_table", "hdr.ethernet.dstAddr") # 2
